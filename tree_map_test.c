...
int main(){
	...
	tree_map_add(a, "Crafton", map);
	tree_map_add(b, "O'brien", map);
	tree_map_add(c, "Hoadly", map);

	TreeMap* map1 = tree_map_constructor(&int_compare);
	tree_map_add(toPointer(10), toPointer(13), map1);
	tree_map_add(toPointer(11), toPointer(14), map1);
	tree_map_add(toPointer(12), toPointer(15), map1);
